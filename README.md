...
`terraform apply` 중 ACM 인증서 DNS validation이 완료될 때까지 최대 **30분** 대기합니다.
에러가 아니므로 그대로 기다리면 됩니다.

### 2. 보안 그룹 전 포트 오픈 상태
현재 개발 편의 목적으로 단일 보안 그룹에 전 포트 오픈 상태입니다.
실제 서비스 배포 전 반드시 보안 그룹 분리가 필요합니다.

### 3. EBS service EC2 용량 부족 가능성
기본 8GiB로 설정되어 있으나, 도커 이미지 11개 + 로그 누적 시 부족할 수 있습니다.
서비스 운영 시작 후 모니터링하며 필요 시 확장하세요. (무중단 확장 가능)
